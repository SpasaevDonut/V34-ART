...
namespace
{
	bool MirvInputXmlReadAttribute(const char * begin, const char * end,
	...
	double MirvInputXmlReadDouble(const char * begin, const char * end,
		const char * name, double fallback)
	{
		std::string value;
		return MirvInputXmlReadAttribute(begin, end, name, value)
			? atof(value.c_str()) : fallback;
	}
}
...
bool MirvInput::Mem::Save(wchar_t const * fileName)
{
	...
	for (std::map<std::string, CData>::iterator it = m_Map.begin(); it != m_Map.end(); ++it)
	{
		char const * name = it->first.c_str();
		CData & data = it->second;
		// State names are console tokens in the original command grammar and
		// therefore cannot contain quotes.
		fprintf(file,
			"<s name=\"%s\" x=\"%.17g\" y=\"%.17g\" z=\"%.17g\" "
			"rx=\"%.17g\" ry=\"%.17g\" rz=\"%.17g\" fov=\"%.17g\" />\n",
			name,
			data.Origin[0], data.Origin[1], data.Origin[2],
			data.Angles[1], data.Angles[0], data.Angles[2], data.Fov);
	}

	const bool ok = 0 <= fprintf(file, "</states>\n</mirvInput>\n") && !ferror(file);
	fclose(file);
	return ok;
}
...
bool MirvInput::Mem::Load(wchar_t const * fileName)
{
	...
	const bool readOk = fileSize == fread(&data[0], sizeof(char), fileSize, pFile);
	fclose(pFile);
	if (!readOk || !strstr(&data[0], "<mirvInput"))
		return false;

	const char * state = &data[0];
	bool loadedAny = false;
	while ((state = strstr(state, "<s ")) != 0)
	{
		const char * stateEnd = strchr(state, '>');
		if (!stateEnd)
			return false;
		std::string name;
		if (MirvInputXmlReadAttribute(state, stateEnd, "name", name))
		{
			const double x = MirvInputXmlReadDouble(state, stateEnd, "x", 0.0);
			const double y = MirvInputXmlReadDouble(state, stateEnd, "y", 0.0);
			const double z = MirvInputXmlReadDouble(state, stateEnd, "z", 0.0);
			const double rx = MirvInputXmlReadDouble(state, stateEnd, "rx", 0.0);
			const double ry = MirvInputXmlReadDouble(state, stateEnd, "ry", 0.0);
			const double rz = MirvInputXmlReadDouble(state, stateEnd, "rz", 0.0);
			const double fov = MirvInputXmlReadDouble(state, stateEnd, "fov", 90.0);
			m_Map[name] = CData(x, y, z, ry, rx, rz, fov);
			loadedAny = true;
		}
		state = stateEnd + 1;
	}
...
{
	// ART's SetCursorPos hook passes the actual post-warp coordinates. Using
	// them directly avoids re-entering the hooked GetCursorPos function.
	m_LastCursorX = x;
	m_LastCursorY = y;
}
